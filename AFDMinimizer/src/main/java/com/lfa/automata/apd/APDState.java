	...
	private void popPush(APDTransition apdTrans, Stack<String> stack) {
		apdTrans.getToPop().reverse().forEach(pop -> {
			if (!StringUtils.equals(pop, stack.pop())) {
				throw new ValidationException(buildPopErrorMessage(pop, stack));
			}
		});
		apdTrans.getToPush().forEach(push -> stack.push(push));
	}

	private String buildPopErrorMessage(String pop, Stack<String> stack) {
		stack.push(pop);
		return "Não foi possível desempilhar " + pop + " no estado " + getName() + Constants.NEWLINE + "Stack no momento: " + stack.toString() + ".";
	}
}
